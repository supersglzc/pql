...
def create_buffer(capacity, obs_dim, action_dim, device='cuda', reserve_space=False):
    if isinstance(capacity, int):
        capacity = (capacity,)
    buf_obs_size = (*capacity, obs_dim) if isinstance(obs_dim, int) else (*capacity, *obs_dim)
    buf_obs = torch.empty(buf_obs_size,
                          dtype=torch.float16 if reserve_space else torch.float32, device='cpu' if reserve_space else device)
...
class ReplayBuffer:
    def __init__(self, capacity: int, obs_dim: int, action_dim: int, device='cpu', left_agent: bool = False, reserve_space: bool = False):
        ...
        ret = create_buffer(capacity=self.capacity, obs_dim=obs_dim, action_dim=action_dim, device=device, reserve_space=reserve_space)
        ...
        if self.left_agent:
            self.buf_reward_left = torch.empty_like(self.buf_reward, device=device)

    @torch.no_grad()
    def add_to_buffer(self, trajectory):
        if self.left_agent:
            obs, actions, rewards, next_obs, dones, rewards_left = trajectory
            rewards_left = rewards_left.reshape(-1, 1)
        else:
            obs, actions, rewards, next_obs, dones = trajectory
        obs = obs.reshape(-1, *self.obs_dim).to('cpu')
        actions = actions.reshape(-1, self.action_dim)
        rewards = rewards.reshape(-1, 1)
        next_obs = next_obs.reshape(-1, *self.obs_dim).to('cpu')
        ...
        if p > self.capacity:
            ...
            if self.left_agent:
                self.buf_reward_left[self.next_p:self.capacity] = rewards_left[:self.capacity - self.next_p]
            ...
            if self.left_agent:
                self.buf_reward_left[0:p] = rewards_left[-p:]
            ...
            if self.left_agent:
                self.buf_reward_left[self.next_p:p] = rewards_left
    ...
    def sample_batch(self, batch_size, device='cuda'):
        ...
        cpu_indices = indices.clone().to('cpu')
        if self.left_agent:
            return (
            self.buf_obs[cpu_indices if self.reserve_space else indices].to(device).to(torch.float32),
            self.buf_action[indices].to(device),
            self.buf_reward[indices].to(device),
            self.buf_next_obs[cpu_indices if self.reserve_space else indices].to(device).to(torch.float32),
            self.buf_done[indices].to(device).float(),  
            self.buf_reward_left[indices].to(device)
        )
        return (
