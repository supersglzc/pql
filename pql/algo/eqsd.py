...
class AgentEQSD(ActorCriticBase):
    def __post_init__(self):
        super().__post_init__()
        self.algo_multi_cfg = self.cfg.task.multi.EQSD
        self.obs_dim = list(self.cfg.task.multi.EQSD.single_agent_obs_dim)
        self.action_dim = int(self.cfg.task.multi.EQSD.single_agent_action_dim)
        ...
        self.symmetry_manager = SymmetryManager(self.cfg.task.multi.EQSD, self.cfg.task.symmetry.symmetric_envs)
    ...
    def explore_env(self, env, timesteps: int, random: bool = False) -> list:
        ...
        traj_obs_combine = torch.zeros((timesteps, self.cfg.num_envs) + (*obs_dim_team,), device=self.device)
        ...
        traj_actions_combine = torch.zeros((timesteps, self.cfg.num_envs) + (self.action_dim*2,), device=self.device)
        ...
        for step in range(timesteps):
            ...
            traj_obs_combine[step] = deepcopy(ob_team)
            ...
            traj_actions_combine[step] = action
        ...
        b_obs_combine = traj_obs_combine.reshape((-1,) + (*obs_dim_team,))
        b_actions_combine = traj_actions_combine.reshape((-1,) + (self.action_dim*2,))
        data_combine = [b_obs_combine, b_actions_combine]
        return [data, data_left, data_combine], timesteps * self.cfg.num_envs
        ...
        for _ in range(self.cfg.algo.update_times):
            ...
            for start in range(0, buffer_size, self.cfg.algo.batch_size):
                ...
                if self.cfg.algo.obs_norm:
                    ...
                    obs_combine = b_obs_combine[mb_inds]
                ...
                if self.cfg.task.multi.same_policy:
                    ...
                    actor_loss = actor_loss - self.cfg.algo.lambda_entropy * entropy.mean()
                    actor_loss_left = actor_loss_left - self.cfg.algo.lambda_entropy * entropy_left.mean()
                    actor_loss_team = imitation_loss
