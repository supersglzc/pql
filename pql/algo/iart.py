...
@dataclass
class AgentIART(ActorCriticBase):
    def __post_init__(self):
        super().__post_init__()
        self.algo_multi_cfg = self.cfg.task.multi.IART
        self.obs_dim = list(self.cfg.task.multi.IART.single_agent_obs_dim)
        self.action_dim = int(self.cfg.task.multi.IART.single_agent_action_dim)
        ...
        self.actor = act_class(self.G, self.cfg.task.symmetry.EQS.actor_input_fields[0], self.cfg.task.symmetry.EQS.actor_output_fields[0], self.obs_dim[0], self.action_dim).to(self.cfg.device)
        self.actor_left = act_class(self.G, self.cfg.task.symmetry.EQS.actor_input_fields[1], self.cfg.task.symmetry.EQS.actor_output_fields[1], self.obs_dim[1], self.action_dim).to(self.cfg.device)
        self.actor_team = act_class(self.G, self.cfg.task.symmetry.EQS.actor_input_fields[0], self.cfg.task.symmetry.EQS.actor_output_fields[0], self.obs_dim[0], self.action_dim).to(self.cfg.device)
        self.actor_left_team = act_class(self.G, self.cfg.task.symmetry.EQS.actor_input_fields[1], self.cfg.task.symmetry.EQS.actor_output_fields[1], self.obs_dim[1], self.action_dim).to(self.cfg.device)

        self.critic = cri_class(self.G, self.cfg.task.symmetry.EQS.critic_input_fields[0], self.cfg.task.symmetry.EQS.critic_output_fields[0], self.obs_dim[0], 1).to(self.cfg.device)
        self.critic_left = cri_class(self.G, self.cfg.task.symmetry.EQS.critic_input_fields[1], self.cfg.task.symmetry.EQS.critic_output_fields[1], self.obs_dim[1], 1).to(self.cfg.device)
        self.critic_team = cri_class(self.G, self.cfg.task.symmetry.EQS.critic_input_fields[0], self.cfg.task.symmetry.EQS.critic_output_fields[0], self.obs_dim[0], 1).to(self.cfg.device)
        self.critic_left_team = cri_class(self.G, self.cfg.task.symmetry.EQS.critic_input_fields[1], self.cfg.task.symmetry.EQS.critic_output_fields[1], self.obs_dim[1], 1).to(self.cfg.device)
        ...
        self.actor_optimizer_left_team = torch.optim.AdamW(self.actor_left_team.parameters(), self.cfg.algo.actor_lr)
        ...
        self.critic_optimizer_left_team = torch.optim.AdamW(self.critic_left_team.parameters(), self.cfg.algo.critic_lr)
        ...
        if self.cfg.algo.value_norm:
            ...
            self.value_rms_left_team = RunningMeanStd(shape=(1), device=self.device)

        self.symmetry_manager = SymmetryManager(self.cfg.task.multi.IART, self.cfg.task.symmetry.symmetric_envs)
    ...
    def explore_env(self, env, timesteps: int, random: bool = False) -> list:
        ...
        traj_obs_team = torch.zeros((timesteps, self.cfg.num_envs//2) + (*obs_dim,), device=self.device)
        traj_obs_left_team = torch.zeros((timesteps, self.cfg.num_envs//2) + (*obs_dim_left,), device=self.device)
        ...
        traj_actions_team = torch.zeros((timesteps, self.cfg.num_envs//2) + (self.action_dim,), device=self.device)
        traj_actions_left_team = torch.zeros((timesteps, self.cfg.num_envs//2) + (self.action_dim,), device=self.device)
        ...
        traj_logprobs_team_side = torch.zeros((timesteps, self.cfg.num_envs//2), device=self.device)
        traj_logprobs_left_team_side = torch.zeros((timesteps, self.cfg.num_envs//2), device=self.device)
        traj_logprobs_team = torch.zeros((timesteps, self.cfg.num_envs//2), device=self.device)
        traj_logprobs_left_team = torch.zeros((timesteps, self.cfg.num_envs//2), device=self.device)
        ...
        traj_rewards_team_side = torch.zeros((timesteps, self.cfg.num_envs//2), device=self.device)
        traj_rewards_left_team_side = torch.zeros((timesteps, self.cfg.num_envs//2), device=self.device)
        traj_rewards_team = torch.zeros((timesteps, self.cfg.num_envs//2), device=self.device)
        traj_rewards_left_team = torch.zeros((timesteps, self.cfg.num_envs//2), device=self.device)
        ...
        traj_values_team_side = torch.zeros((timesteps, self.cfg.num_envs//2), device=self.device)
        traj_values_left_team_side = torch.zeros((timesteps, self.cfg.num_envs//2), device=self.device)
        traj_values_team = torch.zeros((timesteps, self.cfg.num_envs//2), device=self.device)
        traj_values_left_team = torch.zeros((timesteps, self.cfg.num_envs//2), device=self.device)
        infos = []
        ...
        for step in range(timesteps):
            ...
            traj_obs_team[step] = deepcopy(ob_right[self.cfg.num_envs//2:])
            traj_obs_left_team[step] = deepcopy(ob_left[self.cfg.num_envs//2:])
            ...
            action_right_team, logprob_right_team, val_right_team = self.get_actions(ob_right[self.cfg.num_envs//2:], self.actor_team, self.critic_team, self.value_rms_team)
            action_left_team, logprob_left_team, val_left_team = self.get_actions(ob_left[self.cfg.num_envs//2:], self.actor_left_team, self.critic_left_team, self.value_rms_left_team)
            action_right_execute = torch.cat([action_right, action_right_team], dim=0)
            action_left_execute = torch.cat([action_left, action_left_team], dim=0)
            action = self.symmetry_manager.get_execute_action(action_right_execute, action_left_execute, cur_symmetry_tracker)
            # get logprob for another side
            logprob_right_team_side = self.actor.logprob(ob_right[self.cfg.num_envs//2:], action_right_team)
            logprob_left_team_side = self.actor_left.logprob(ob_left[self.cfg.num_envs//2:], action_left_team)
            val_right_team_side = self.get_values(ob_right[self.cfg.num_envs//2:], self.critic, self.value_rms)
            val_left_team_side = self.get_values(ob_left[self.cfg.num_envs//2:], self.critic_left, self.value_rms_left)
            next_ob, reward, done, info = env.step(action)

            reward_right, reward_left, reward_right_team, reward_left_team = self.symmetry_manager.get_multi_agent_rew(info['detailed_reward'], cur_symmetry_tracker)
            ...
            traj_actions_team[step] = action_right_team
            traj_actions_left_team[step] = action_left_team

            traj_logprobs[step] = logprob_right
            traj_logprobs_left[step] = logprob_left
            traj_logprobs_team_side[step] = logprob_right_team_side
            traj_logprobs_left_team_side[step] = logprob_left_team_side
            traj_logprobs_team[step] = logprob_right_team
            traj_logprobs_left_team[step] = logprob_left_team

            traj_rewards[step] = reward_right[:self.cfg.num_envs//2]
            traj_rewards_left[step] = reward_left[:self.cfg.num_envs//2]
            traj_rewards_team_side[step] = reward_right[self.cfg.num_envs//2:]
            traj_rewards_left_team_side[step] = reward_left[self.cfg.num_envs//2:]
            traj_rewards_team[step] = reward_right_team[self.cfg.num_envs//2:]
            traj_rewards_left_team[step] = reward_left_team[self.cfg.num_envs//2:]

            traj_values[step] = val_right
            traj_values_left[step] = val_left
            traj_values_team_side[step] = val_right_team_side
            traj_values_left_team_side[step] = val_left_team_side
            traj_values_team[step] = val_right_team
            traj_values_left_team[step] = val_left_team
        ...
        data_team = self.compute_adv((traj_obs_team, traj_actions_team, traj_logprobs_team, traj_rewards_team,
                                 traj_dones_team, traj_values_team, ob_right[self.cfg.num_envs//2:], dones[self.cfg.num_envs//2:]), gae=self.cfg.algo.use_gae, timeout=self.timeout_info[:, self.cfg.num_envs//2:], critic=self.critic_team, value_rms=self.value_rms_team)
        data_left_team = self.compute_adv((traj_obs_left_team, traj_actions_left_team, traj_logprobs_left_team, traj_rewards_left_team,
                                 traj_dones_team, traj_values_left_team, ob_left[self.cfg.num_envs//2:], dones[self.cfg.num_envs//2:]), gae=self.cfg.algo.use_gae, timeout=self.timeout_info[:, self.cfg.num_envs//2:], critic=self.critic_left_team, value_rms=self.value_rms_left_team)

        data_team_side = self.compute_adv((deepcopy(traj_obs_team), None, traj_logprobs_team_side, traj_rewards_team_side,
                                 traj_dones_team, traj_values_team_side, ob_right[self.cfg.num_envs//2:], dones[self.cfg.num_envs//2:]), gae=self.cfg.algo.use_gae, timeout=self.timeout_info[:, self.cfg.num_envs//2:], critic=self.critic, value_rms=self.value_rms)
        data_left_team_side = self.compute_adv((deepcopy(traj_obs_left_team), None, traj_logprobs_left_team_side, traj_rewards_left_team_side,
                                 traj_dones_team, traj_values_left_team_side, ob_left[self.cfg.num_envs//2:], dones[self.cfg.num_envs//2:]), gae=self.cfg.algo.use_gae, timeout=self.timeout_info[:, self.cfg.num_envs//2:], critic=self.critic_left, value_rms=self.value_rms_left)
        return [data, data_left, data_team, data_left_team, data_team_side, data_left_team_side], timesteps * self.cfg.num_envs
    ...
    def compute_actor_loss(self, policy, obs, old_actions, old_logprobs, advantages, policy2, obs2, old_actions2, old_logprobs2, advantages2):
        ...
        if policy2 is not None:
            ...
            _, _, newlogprob_off_team_side, _ = policy2.logprob_entropy(obs2, old_actions2)
            logratio_off = newlogprob_off - newlogprob_off_team_side
            miu = old_logprobs2 - newlogprob_off_team_side
    ...
    def update_net(self, data):
        ...
        b_obs_left_team, b_actions_left_team, b_logprobs_left_team, b_advantages_left_team, b_returns_left_team, b_values_left_team = data[3]

        b_obs_team_side, _, b_logprobs_team_side, b_advantages_team_side, _, _ = data[4]
        b_obs_left_team_side, _, b_logprobs_left_team_side, b_advantages_left_team_side, _, _ = data[5]
        ...
        critic_loss_list_left_team = list()
        actor_loss_list_left_team = list()
        for _ in range(self.cfg.algo.update_times):
            ...
            for start in range(0, buffer_size, self.cfg.algo.batch_size):
                ...
                if self.cfg.algo.obs_norm:
                    ...
                    obs_left_team = b_obs_left_team[mb_inds]
                actor_loss, entropy = self.compute_actor_loss(self.actor, obs, b_actions[mb_inds], b_logprobs[mb_inds], b_advantages[mb_inds], 
                                                              self.actor_team, b_obs_team_side[mb_inds], b_actions_team[mb_inds], b_logprobs_team_side[mb_inds], b_advantages_team_side[mb_inds])
                actor_loss_left, entropy_left = self.compute_actor_loss(self.actor_left, obs_left, b_actions_left[mb_inds], b_logprobs_left[mb_inds], b_advantages_left[mb_inds], 
                                                                        self.actor_left_team, b_obs_left_team_side[mb_inds], b_actions_left_team[mb_inds], b_logprobs_left_team_side[mb_inds], b_advantages_left_team_side[mb_inds])
                actor_loss_team, entropy_team = self.compute_actor_loss(self.actor_team, obs_team, b_actions_team[mb_inds], b_logprobs_team[mb_inds], b_advantages_team[mb_inds], None, None, None, None, None)
                actor_loss_left_team, entropy_left_team = self.compute_actor_loss(self.actor_left_team, obs_left_team, b_actions_left_team[mb_inds], b_logprobs_left_team[mb_inds], b_advantages_left_team[mb_inds], None, None, None, None, None)
                ...
                critic_loss_left_team = self.compute_critic_loss(self.critic_left_team, obs_left_team, b_returns_left_team[mb_inds], b_values_left_team[mb_inds])
                ...
                if self.cfg.task.multi.same_policy:
                    ...
                    actor_loss_left_team = actor_loss_left_team - self.cfg.algo.lambda_entropy * entropy_left_team.mean()
                    ...
                    self.optimizer_update(self.actor_optimizer_left_team, actor_loss_left_team)
                    self.optimizer_update(self.critic_optimizer_left_team, critic_loss_left_team)
                    critic_loss_list.append(critic_loss.item())
                    actor_loss_list.append(actor_loss.item())
                    ...
                    critic_loss_list_left_team.append(critic_loss_left_team.item())
                    actor_loss_list_left_team.append(actor_loss_left_team.item())
        ...
        log_info = {
            ...
            "train/critic_loss_left_team": np.mean(critic_loss_list_left_team),
            "train/actor_loss_left_team": np.mean(actor_loss_list_left_team),
