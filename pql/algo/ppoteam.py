...
@dataclass
class AgentIPPOTeam(ActorCriticBase):
    ...
    def explore_env(self, env, timesteps: int, random: bool = False) -> list:
        ...
        traj_actions_tot = torch.zeros((timesteps, self.cfg.num_envs) + (self.action_dim*2,), device=self.device)
        ...
        traj_actions_other = torch.zeros((timesteps, self.cfg.num_envs//2) + (self.action_dim,), device=self.device)
        traj_actions_left_other = torch.zeros((timesteps, self.cfg.num_envs//2) + (self.action_dim,), device=self.device)
        ...
        for step in range(timesteps):
            ...
            action_executed = torch.cat([action_ind[:self.cfg.num_envs//2], action_team], dim=0)
            ...
            traj_actions_other[step] = action_team[:, :22]
            traj_actions_left_other[step] = action_team[:, 22:]
        ...
        data, data_other = self.compute_adv((traj_obs, traj_actions, traj_logprobs, traj_rewards,
                                 traj_dones, traj_values, ob_right, dones), gae=self.cfg.algo.use_gae, timeout=self.timeout_info, critic=self.critic, value_rms=self.value_rms, split=True)
        data_left, data_left_other = self.compute_adv((traj_obs_left, traj_actions_left, traj_logprobs_left, traj_rewards_left,
                                 traj_dones, traj_values_left, ob_left, dones), gae=self.cfg.algo.use_gae, timeout=self.timeout_info, critic=self.critic_left, value_rms=self.value_rms_left, split=True)
        data_tot, data_tot_other = self.compute_adv((traj_obs_tot, traj_actions_tot, None, traj_rewards_tot,
                                 traj_dones, traj_values_tot, ob, dones), gae=self.cfg.algo.use_gae, timeout=self.timeout_info, critic=self.critic_tot, value_rms=self.value_rms_tot, split=True)
        ...
        action_other = traj_actions_other.reshape(-1, self.action_dim)
        action_left_other = traj_actions_left_other.reshape(-1, self.action_dim)
        data_action_other = [action_other, action_left_other]
        return [data, data_left, data_tot, data_team, data_other, data_left_other, data_tot_other, data_action_other], timesteps * self.cfg.num_envs

    def compute_adv(self, buffer, gae=True, timeout=None, critic=None, value_rms=None, split=False):
        with torch.no_grad():
        ...
        if not split:
            ...
            b_obs = obs[:, :self.cfg.num_envs//2].reshape(-1, *obs_dim)
            b_obs_other = obs[:, self.cfg.num_envs//2:].reshape(-1, *obs_dim)
            b_actions = actions[:, :self.cfg.num_envs//2].reshape(-1, action_dim) if actions is not None else None
            b_actions_other = actions[:, self.cfg.num_envs//2:].reshape(-1, action_dim) if actions is not None else None
            b_logprobs = logprobs[:, :self.cfg.num_envs//2].reshape(-1) if logprobs is not None else None
            b_logprobs_other = logprobs[:, self.cfg.num_envs//2:].reshape(-1) if logprobs is not None else None
            b_advantages = advantages[:, :self.cfg.num_envs//2].reshape(-1)
            b_advantages_other = advantages[:, self.cfg.num_envs//2:].reshape(-1)

            if self.cfg.algo.value_norm:
                value_rms.update(returns.reshape(-1))
                b_returns = value_rms.normalize(returns[:, :self.cfg.num_envs//2].reshape(-1))
                b_returns_other = value_rms.normalize(returns[:, self.cfg.num_envs//2:].reshape(-1))
                value_rms.update(values.reshape(-1))
                b_values = value_rms.normalize(values[:, :self.cfg.num_envs//2].reshape(-1))
                b_values_other = value_rms.normalize(values[:, self.cfg.num_envs//2:].reshape(-1))
            else:
                b_returns = returns[:, :self.cfg.num_envs//2].reshape(-1)
                b_returns_other = returns[:, self.cfg.num_envs//2:].reshape(-1)
                b_values = values[:, :self.cfg.num_envs//2].reshape(-1)
                b_values_other = values[:, self.cfg.num_envs//2:].reshape(-1)
            return (b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_values), (b_obs_other, b_actions_other, b_logprobs_other, b_advantages_other, b_returns_other, b_values_other)
    ...
    def update_net(self, data):
        ...
        b_obs_other, _, b_logprobs_other, b_advantages_other, b_returns_other, b_values_other = data[4]
        b_obs_left_other, _, b_logprobs_left_other, b_advantages_left_other, b_returns_left_other, b_values_left_other = data[5]
        b_obs_tot_other, b_actions_tot_other, b_logprobs_tot_other, b_advantages_tot_other, b_returns_tot_other, b_values_tot_other = data[6]
        b_actions_other, b_actions_left_other = data[7]
        ...
        for _ in range(self.cfg.algo.update_times):
            ...
            for start in range(0, buffer_size, self.cfg.algo.batch_size):
                ...
                # # team data
                # _, _, newlogprob_other, _ = self.actor.logprob_entropy(b_obs_other[mb_inds], b_actions_other[mb_inds])
                # _, _, newlogprob_left_other, _ = self.actor_left.logprob_entropy(b_obs_left_other[mb_inds], b_actions_left_other[mb_inds])
                # logratio_other = (newlogprob_other + newlogprob_left_other) - b_logprobs_team[mb_inds]
                # ratio_other = logratio_other.exp()
                # mb_advantages_other = b_advantages_other[mb_inds]
                # mb_advantages_left_other = b_advantages_left_other[mb_inds]
                # mb_advantages_tot_other = b_advantages_tot_other[mb_inds]
                # mb_advantages_other = (mb_advantages_other - mb_advantages_other.mean()) / (mb_advantages_other.std() + 1e-8)
                # mb_advantages_left_other = (mb_advantages_left_other - mb_advantages_left_other.mean()) / (mb_advantages_left_other.std() + 1e-8)
                # mb_advantages_tot_other = (mb_advantages_tot_other - mb_advantages_tot_other.mean()) / (mb_advantages_tot_other.std() + 1e-8)
                # ratio = torch.cat([ratio, ratio_other], dim=0)
                # ratio_left = torch.cat([ratio_left, ratio_other], dim=0)
                # mb_advantages = torch.cat([mb_advantages, mb_advantages_other], dim=0)
                # mb_advantages_left = torch.cat([mb_advantages_left, mb_advantages_left_other], dim=0)
                # mb_advantages_tot = torch.cat([mb_advantages_tot, mb_advantages_tot_other], dim=0)
                ...
                if self.cfg.task.multi.same_policy:
                    ...
                    actor_loss = actor_loss + actor_loss_tot - self.cfg.algo.lambda_entropy
                    actor_loss_left = actor_loss_left + actor_loss_left_tot - self.cfg.algo.lambda_entropy
                    actor_loss_team = actor_loss_team + 0.01 * imitation_loss - self.cfg.algo.lambda_entropy
                    self.optimizer_update(self.actor_optimizer, actor_loss, retain_graph=True)
