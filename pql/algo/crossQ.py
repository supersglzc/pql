...
@dataclass
class AgentCrossQ(ActorCriticBase):
    ...
    def update_critic(self, obs, action, reward, next_obs, done):
        ...
        all_Q1, all_Q2 = self.critic.get_q1_q2(torch.cat((obs, next_obs), dim=0), torch.cat((action, next_actions), dim=0))
        current_Q1, current_Q2 = all_Q1[:len(obs)], all_Q2[:len(obs)]
        next_Q1, next_Q2 = all_Q1[len(obs):].detach(), all_Q2[len(obs):].detach()
