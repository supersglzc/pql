...
@dataclass
class AgentEQSC(ActorCriticBase):
    def __post_init__(self):
        super().__post_init__()
        self.algo_multi_cfg = self.cfg.task.multi.EQSC
        self.obs_dim = list(self.cfg.task.multi.EQSC.single_agent_obs_dim)
        self.action_dim = int(self.cfg.task.multi.EQSC.single_agent_action_dim)
        ...
        self.actor = act_class(self.G, self.cfg.task.symmetry.EQSC.actor_input_fields[0], self.cfg.task.symmetry.EQSC.actor_output_fields[0], self.obs_dim[0], self.action_dim).to(self.cfg.device)
        self.actor_left = act_class(self.G, self.cfg.task.symmetry.EQSC.actor_input_fields[1], self.cfg.task.symmetry.EQSC.actor_output_fields[1], self.obs_dim[1], self.action_dim).to(self.cfg.device)
    
        self.critic = cri_class(self.G, self.cfg.task.symmetry.EQSC.critic_input_fields[0], self.cfg.task.symmetry.EQSC.critic_output_fields[0], self.obs_dim[2], 1).to(self.cfg.device)
        ...
        self.symmetry_manager = SymmetryManager(self.cfg.task.multi.EQSC, self.cfg.task.symmetry.symmetric_envs)
    ...
    def explore_env(self, env, timesteps: int, random: bool = False) -> list:
        ...
        obs_dim_critic = (self.obs_dim[2],) if isinstance(self.obs_dim[2], int) else self.obs_dim[2]
        ...
        traj_obs_critic = torch.zeros((timesteps, self.cfg.num_envs) + (*obs_dim_critic,), device=self.device)
        ...
        for step in range(timesteps):
            ...
            ob_right, ob_left, ob_critic = self.symmetry_manager.get_multi_agent_obs(ob, cur_symmetry_tracker)
            traj_obs[step] = deepcopy(ob_right)
            traj_obs_left[step] = deepcopy(ob_left)
            traj_obs_critic[step] = deepcopy(ob_critic)
            traj_dones[step] = dones

            action_right, logprob_right, val_right = self.get_actions(ob_right, self.actor)
            action_left, logprob_left, val_left = self.get_actions(ob_left, self.actor_left)
            val_critic = self.get_values(ob_critic, self.critic, self.value_rms)
        ...
        ob_right, ob_left, ob_critic = self.symmetry_manager.get_multi_agent_obs(ob, env.unwrapped.symmetry_tracker)
        data = self.compute_adv((traj_obs_critic, None, None, traj_rewards,
                                 traj_dones, traj_values, ob_critic, dones), gae=self.cfg.algo.use_gae, timeout=self.timeout_info, critic=self.critic, value_rms=self.value_rms)

        data_right = [traj_obs.reshape(-1, *obs_dim), traj_actions.reshape(-1, self.action_dim), traj_logprobs.reshape(-1)]
        data_left = [traj_obs_left.reshape(-1, *obs_dim_left), traj_actions_left.reshape(-1, self.action_dim), traj_logprobs_left.reshape(-1)]
        return [data, data_right, data_left], timesteps * self.cfg.num_envs
    ...
    def update_net(self, data):
        b_obs_critic, _, _, b_advantages, b_returns, b_values = data[0]
        b_obs, b_actions, b_logprobs = data[1]
        b_obs_left, b_actions_left, b_logprobs_left = data[2]
        ...
        for _ in range(self.cfg.algo.update_times):
            ...
            for start in range(0, buffer_size, self.cfg.algo.batch_size):
                ...
                if self.cfg.algo.obs_norm:
                    ...
                    obs_critic = b_obs_critic[mb_inds]
