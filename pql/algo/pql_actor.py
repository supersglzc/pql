...
class PQLActor:
    ...
    def explore_env(self, env, timesteps: int, random: bool) -> list:
        ...
        traj_states = torch.empty((self.cfg.num_envs, timesteps) + (*obs_dim,)).to(self.sim_device)
        traj_actions = torch.empty((self.cfg.num_envs, timesteps) + (self.action_dim,)).to(self.sim_device)
        traj_rewards = torch.empty((self.cfg.num_envs, timesteps)).to(self.sim_device)
        traj_next_states = torch.empty((self.cfg.num_envs, timesteps) + (*obs_dim,)).to(self.sim_device)
        traj_dones = torch.empty((self.cfg.num_envs, timesteps)).to(self.sim_device)
    ...
    def update_tracker(self, reward, done):
        ...
        time_out_dones = torch.where(self.current_lengths == self.env.max_episode_length)
