...
from pql.utils.common import aggregate_traj_info
...
class AgentPPO(ActorCriticBase):
    def __post_init__(self):
        super().__post_init__()
        self.done = torch.zeros(self.cfg.num_envs).to(self.device)
    ...
    def explore_env(self, env, timesteps: int, random: bool = False) -> list:
        ...
        traj_obs = torch.zeros((timesteps, self.cfg.num_envs) + (*obs_dim,)).to(self.device)
        traj_actions = torch.zeros((timesteps, self.cfg.num_envs) + (self.action_dim,)).to(self.device)
        traj_logprobs = torch.zeros((timesteps, self.cfg.num_envs)).to(self.device)
        traj_rewards = torch.zeros((timesteps, self.cfg.num_envs)).to(self.device)
        traj_dones = torch.zeros((timesteps, self.cfg.num_envs)).to(self.device)
        traj_values = torch.zeros((timesteps, self.cfg.num_envs)).to(self.device)
        infos = []
        ...
        for i in range(timesteps):
            ...
            actions, logprob, values = self.get_actions(obs)

            next_obs, rewards, next_dones, info = env.step(actions)
            traj_actions[i] = actions
            traj_logprobs[i] = logprob
            traj_rewards[i] = rewards
            traj_values[i] = values
            infos.append(deepcopy(info))
            obs, dones = next_obs, next_dones
            self.update_tracker(rewards, dones)
        ...
        data = self.compute_adv((traj_obs, traj_actions, traj_logprobs, traj_rewards * self.cfg.algo.reward_scale,
                                 traj_dones, traj_values, obs, dones), gae=self.cfg.algo.use_gae, timeout=self.timeout_info)
        ...
        with torch.no_grad():
            ...
            if gae:
                ...
                for t in reversed(range(timesteps)):
                    ...
                    advantages[t] = lastgaelam = delta + self.cfg.algo.gamma * self.cfg.algo.lambda_gae_adv * nextnonterminal * lastgaelam
