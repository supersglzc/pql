...
@dataclass
class AgentEQ(ActorCriticBase):
    def __post_init__(self):
        super().__post_init__()
        self.algo_multi_cfg = self.cfg.task.multi.EQ
        self.obs_dim = list(self.cfg.task.multi.EQ.single_agent_obs_dim)
        self.action_dim = int(self.cfg.task.multi.EQ.single_agent_action_dim)
        ...
        self.actor = act_class(self.G, self.cfg.task.symmetry.EQ.actor_input_fields[0], self.cfg.task.symmetry.EQ.actor_output_fields[0], self.obs_dim[0], self.action_dim).to(self.cfg.device)

        self.critic = cri_class(self.G, self.cfg.task.symmetry.EQ.critic_input_fields[0], self.cfg.task.symmetry.EQ.critic_output_fields[0], self.obs_dim[0], 1).to(self.cfg.device)
        ...
        self.symmetry_manager = SymmetryManager(self.cfg.task.multi.EQ, self.cfg.task.symmetry.symmetric_envs)
    ...
    def explore_env(self, env, timesteps: int, random: bool = False) -> list:
        ...
        for step in range(timesteps):
            ...
            action_left, logprob_left, val_left = self.get_actions(ob_left, self.actor, self.critic, self.value_rms)
        ...
        data_left = self.compute_adv((traj_obs_left, traj_actions_left, traj_logprobs_left, traj_rewards_left,
                                 traj_dones, traj_values_left, ob_left, dones), gae=self.cfg.algo.use_gae, timeout=self.timeout_info, critic=self.critic, value_rms=self.value_rms)
    ...
    def update_net(self, data):
        ...
        for _ in range(self.cfg.algo.update_times):
            ...
            for start in range(0, buffer_size, self.cfg.algo.batch_size):
                ...
                _, action_dist_left, newlogprob_left, entropy_left = self.actor.logprob_entropy(obs_left, b_actions_left[mb_inds])
                ...
                newvalue_left = self.critic(obs_left)
